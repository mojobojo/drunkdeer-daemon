...
pub fn set_led_by_key_id(device: &HidDevice, key_id: u8, color_r: u8, color_g: u8, color_b: u8, brightness: u8) {
    let data: [u8; 14] = [
        0x04,
        0xAE,
        0x01,
    ...
    let mut packet = [0u8; 64];
    packet[..14].copy_from_slice(&data);
